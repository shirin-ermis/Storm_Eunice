...
# NJL: include input path as system arg
_, inpath, outpath, suppath, variable = sys.argv[:]

'''*******************************************
Set up Environment
*******************************************'''
print("Setting up environment")
path = "/home/ubuntu/notebooks/wind-eu/preliminary"
dataset = "ERA5"
verd = "12_4Test" # Detection Version
vert = 'Tracks' # Tracking Version
spres = 25 # Spatial resolution (in km)

# inpath = path+"/input/psl_EUR-11_ECMWF-ERAINT_evaluation_r1i1p1_MOHC-HadREM3-GA7-05_v1_3hr_198212010000-198212312100_REGRID.nc"
# outpath = path+"/output"
# suppath = path+"/input/EASE2_N0_25km_Projection_CORDEX.nc"
...
# starttime = [1982,12,1,0,0,0] # Format: [Y,M,D,H,M,S]
# endtime = [1983,1,1,0,0,0] # stop BEFORE this time (exclusive)
# timestep = [0,0,0,3,0,0] # Time step in [Y,M,D,H,M,S]

# dateref = [1949,12,1,0,0,0]  # [Y,M,D,H,M,S]

if os.path.isfile(outpath+'/detection12_4Test/CycloneFields/'+previous_step.strftime('%Y/%h/CF%Y%m%d_%H%M.pkl')):
...
while t != endtime:
    ...
    if t[2] == 1 and t[3] == starting_hour: # If the next timestep is the 0th hour of the 1st day of a month,
        ...
        # NJL: if month is March, skip to October
        if t[1] == 4:
            t[1] = 10
